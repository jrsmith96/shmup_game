...
class Pow(pygame.sprite.Sprite):
    def __init__(self, center):
        pygame.sprite.Sprite.__init__(self)
        self.kind = random.choice(['shield', 'gun'])
        self.image = powerup_images[self.kind]
        self.image.set_colorkey(BLACK)
        self.rect = self.image.get_rect()
...
def show_go_screen():
    ...
    draw_text(screen, "Star Blast!", 64, WIDTH / 2, HEIGHT / 4)
...
while running:
    ...
    for hit in hits:
        score += 50 - hit.radius
        random.choice(expl_sounds).set_volume(0.5)
        random.choice(expl_sounds).play()
        expl = Explosion(hit.rect.center, 'lg')
        all_sprites.add(expl)
        if random.random() > 0.9:
            wop = Pow(hit.rect.center)
            all_sprites.add(wop)
            powerups.add(wop)
        newmob()
